...
\begin{frame}
  ...
  \item[Weight function specification] \mbox{}\\
    \resizebox{!}{1.5ex}{\beamergotobutton{in preparation}}
...
\begin{frame}
  ...
  \only<2->{
    \blankfootnote{
      \hyperlink{frm:integral-riemann}{\resizebox{!}{1.5ex}{\beamergotobutton{appendix}}}
      The Riemann sum is one strategy to evaluate the integral
    }
  }
\end{frame}
...
\begin{frame}
  ...
  \blankfootnote{
    $n = 100$, $\phi = 1$, $\sigma^2_f = 1$,
    and $\sigma^2_\varepsilon = 30^{-1}$,
    $s_x(t_{k}, t_{k'}) = \exp\left\{-.1^{-2} {(t_{k} - t_{k'})}^2 \right\}$.
...
\begin{frame}%
  ...
  \item Draw $N\in\mathbb{N}$ output scalars from
    ...
    % with elements ${(\mathbf{S}_y)}_{i,j} = s_f(X_i(t), X_j(t)) + I_{i, j}
    %\sigma_{\varepsilon}^2$, $i,j = 1, \dots, N$ are observation indexes,
...
\begin{frame}
  ...
  \framesubtitle{Well-specified models have lower prediction error}
  \begin{figure}
    \centering
    \includegraphics[height=10em]{inc/page99_table.pdf}
    \caption{
      RMSE (rows)
      from plausible models (columns)
      mean over eight data splits.
      Bold: generative model.
      Parenthesis: 80\% interval for the mean using $t_{7}\approx1.41$.
      Underline: best in class.
    }
  \end{figure}
\end{frame}
