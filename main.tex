...
\begin{frame}%
  \label{frm:piecewise}
  \frametitle{Integral evaluation with a finite representation}
  \frametitle{Closed-form expression for piecewise input and weight functions}
...
\begin{frame}%
  ...
  Add Riemann sum equivalence equation

  \begin{itemize}
  \item Pointers on how to use it
  \item Train an $d_{\textsc{ARD}}(\cdot)$ model with $\theta_k > 0 \
    \forall \ k = 1, \dots K$ on $\mathbf{X}$
  \item Define the upscale constants $\Delta t_k = t_{k} - t_{k - 1}$
  \item Visualize the upscaled scalar weights
    $\dots$
  \end{itemize}
\end{frame}
